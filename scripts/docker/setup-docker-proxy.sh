...
#       如果不传 proxy_host，会自动用 find-and-set-proxy.sh 查找代理
#
# 需搭配 scripts/proxy/find-and-set-proxy.sh 使用
...
if [ $# -ge 1 ]; then
  ...
  if [ -f "$SCRIPT_DIR/../proxy/find-and-set-proxy.sh" ]; then
    # 复用脚本时指定只扫代理端口
    PROXY_HOST=$(bash "$SCRIPT_DIR/../proxy/find-and-set-proxy.sh" 2>/dev/null | grep 'HTTP proxy' | head -1 | awk '{print $NF}')
  fi
